...
1. BAT가 `input/`의 Excel을 수정일 기준 오래된 순서로 찾는다.
2. DB에 성공한 import가 없으면 모든 과거 파일을 최초 백필한다.
3. 이후에는 파일 해시가 `source_import`에 없는 파일만 처리한다.
4. xlwings가 파일을 숨김·읽기 전용으로 열고 `db.PLC`만 읽는다.
5. 파일 하나를 하나의 SQLite 트랜잭션으로 처리한다.
6. 파일 내 모든 행이 통과해야 commit하고, 실패하면 해당 파일의 변경을 모두 rollback한다.
7. 성공한 파일은 `archive/processed/`로 이동한다.
8. 모든 신규 파일이 성공한 경우에만 JSON을 임시 위치에 생성한 뒤 기존 산출물과 원자적으로 교체한다.
