...
- Excel 파일명은 `- Jun.26` 같은 영문 월·2자리 연도 표기를 포함한다. 이를 `2026-06`으로 정규화한 값이 원본 파일의 기준월과 백필 처리 순서를 결정한다.
...
1. BAT가 `input/`의 Excel 파일명에서 `Jan.YY`부터 `Dec.YY` 형태의 영문 월·2자리 연도 표기를 대소문자 구분 없이 찾고 `20YY-MM`으로 변환한다. 예: `- Jun.26` → `2026-06`.
2. 파일 수정일은 사용하지 않고, 변환한 파일 기준월이 오래된 순서로 정렬한다. 예를 들어 `- Jan.26`을 `- Jun.26`보다 먼저 처리한다.
