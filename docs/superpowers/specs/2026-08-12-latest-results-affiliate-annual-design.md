...
- 기본 선택: `LSI`
- 연도 선택 버튼은 두지 않고 `'24 · '25 · '26 · '27`을 한 표에서 함께 표시한다.
...
1. 우측에서 `LSI/A/B/D/E/F/M` 중 하나를 선택하고, `'24~'27`은 한 표의 열로 동시에 비교한다.
2. 기본값은 `LSI`이며 관계사 표는 승인된 12개 행을 표시한다.
